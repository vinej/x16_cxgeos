; ca65
; =====================================================================
; CXGEOS :: kernel/audio/audio.asm -- PSG + YM audio, in bank 2
; =====================================================================
; The x16lib PSG and YM modules will not fit the resident budget (all of
; audio + sprites + PCM overflows it by 868 bytes), so they ride bank 2
; with the widget/menu code, reached through the five-byte far-call stubs
; in kernel/resident/farcall.asm. The ABI exposes tone control on the
; VERA PSG (16 voices) and note/patch/volume on the YM2151 FM chip.
;
; The gates X16_USE_PSG / X16_USE_YM stay OFF in kernel.asm -- turning
; them on would pull the code into the resident image. Instead the module
; sources are .included here, inside B2CODE, so they compile into bank 2.
;
...
; the x16lib modules themselves, compiled into bank 2 (the gates stay off
; so x16_code.asm does not also place them in the resident image)
