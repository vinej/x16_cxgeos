...
**Commander X16 Runtime Framework** — V0.10.0

Per-version release notes live in [releases.md](releases.md).
...
- `emulator\rom.bin` — **the official stock R49 ROM only**, from the
  ...
  (sha256 `298e3e2a…`). CXRF's whole premise is running on stock ROM.
...
`x16lib/` is a snapshot of `x16_library/src_ca65/` at **v0.10.0** (a rename sync
with this release — CXGEOS→CXRF in comments — over an otherwise unchanged
v0.9.1). Since the v0.7.0 gfx size pass it has gained 0.8.0's polygon/arc/pie
**shapes** and 0.9.0's new module families (serial/WiFi + ZiModem, packed-BCD,
banked stack/ring buffers, the string library — all pay-per-use, none pulled
into the kernel), and 0.9.1's **`X16_SKIP_SHAPES`/`X16_SKIP_MATH`** include
opt-out: the kernel places the shape and trig modules across its own banks (17
and 19), and that flag keeps `x16_code.asm` from sourcing them a second time
(the compatibility hook that let this bump build). The 64tass value-gate model
is generated by the converter, not hand-maintained. Update it by re-copying the
