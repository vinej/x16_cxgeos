...
V0.9.0 — **the desktop grows up, and tiles go 8bpp**. The icon view is now a
real file browser: **eighteen** built-in icons with a fitting picture for each
demo (a calculator on the calculator, a joystick on the game, …); **click or
arrow-key** to move the selection, RETURN to open, and the selection **survives
launching an app** and coming back. Radio buttons are **round**, the prompt
field centres its text, and **TAB toggles** the menu bar. On the game side,
tile mode gained an **8bpp path** — a 64 KB tile set streamed to VRAM
(`cx_vram_stream`, `cx_tile_load`) with the maps relocated to `$10000` to make
room — plus **double-buffering** (`cx_tile_dbuf`/`cx_tile_flip`) and a mode-2
**dialog overlay** (`cx_tile_text`) so a paused tile game can raise the whole
toolkit. ABI is now **v4, 105 slots**, and the vendored x16lib moves to
**0.9.1**.
(v0.8.0 added **Prog8** as a 13th toolchain and x16lib's polygon/arc/pie
**shapes** on one dispatched slot; v0.7.1 the **ca65 macro SDK**; v0.7.0
**invisible hit regions and a palette API**; v0.6.x the per-bank **memory
...
`x16lib/` is a snapshot of `x16_library/src_ca65/` at **v0.9.1**. Since the
v0.7.0 gfx size pass it has gained 0.8.0's polygon/arc/pie **shapes** and
0.9.0's new module families (serial/WiFi + ZiModem, packed-BCD, banked
stack/ring buffers, the string library — all pay-per-use, none pulled into the
kernel), and 0.9.1's **`X16_SKIP_SHAPES`/`X16_SKIP_MATH`** include opt-out: the
kernel places the shape and trig modules across its own banks (17 and 19), and
that flag keeps `x16_code.asm` from sourcing them a second time (the
compatibility hook that let this bump build). The 64tass value-gate model is
generated by the converter, not hand-maintained. Update it by re-copying the
tree and noting the new version here.
