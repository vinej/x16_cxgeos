...
V0.6.1 — a **graphical icon widget and an icon-view desktop**, the way GEOS
lays out a disk: one 24×24 2bpp sheet serves both bitmap modes (mode 0 blits
it, mode 1 expands it to 8bpp), a new `WG_ICON` widget draws icon-plus-label,
and the filer's **View** menu toggles a file-type icon grid against the
classic list. Also new: sprite-collision capture (`cx_spr_collide`). The
toolkit's old 16-widget ceiling is gone.
(v0.6.0 rebuilt the **memory architecture for growth**: kernel code themed one
purpose per bank across two loadable files, banks 2–5 + 16–19, 64 KB up from
32, so a new widget or shape grows exactly one bank; see
...
`x16lib/` is a snapshot of `x16_library/src_ca65/` at **v0.6.1** (c559772:
finer module gates — `VERA`/`IRQ`/`INPUT`/`SCREEN` split into `_CORE` +
optional sub-gates the way `VERAFX` was, so the kernel takes each module's
core and drops what it never calls: `vera_copy`, `vsync_wait`,
`key_wait`/`key_peek`, the SCREEN query/border/cursor/charset/puts
helpers, and the sprite-collision API — keeping only the collision
`CAPTURE` that `cx_spr_collide` reads; the 64tass value-gate model is
generated by the converter, not hand-maintained). The shape/ellipse
routines still ride bank 5 with the rest of the shape machinery; no ABI
slots expose
them yet.
